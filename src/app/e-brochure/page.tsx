        ...
        <main className="flex-grow flex flex-col items-center pt-28 pb-12 px-4 relative overflow-hidden">
          ...
          <div className="w-full max-w-5xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="text-center space-y-4">
              ...
              <h1 className="text-4xl md:text-5xl font-bold font-headline text-primary">E-Brochure</h1>
            ...
            <div className="flex items-center justify-center space-x-6 bg-white/50 backdrop-blur-md p-4 rounded-full shadow-sm border border-primary/10 w-fit mx-auto">
              <div className={`flex items-center space-x-2 cursor-pointer transition-colors ${!isHindi ? 'text-primary' : 'text-muted-foreground'}`} onClick={() => setIsHindi(false)}>
                <Globe className="w-4 h-4" />
                <span className="font-bold">English</span>
              </div>
              <Switch checked={isHindi} onCheckedChange={setIsHindi} className="data-[state=checked]:bg-primary" />
              <div className={`flex items-center space-x-2 cursor-pointer transition-colors ${isHindi ? 'text-primary' : 'text-muted-foreground'}`} onClick={() => setIsHindi(true)}>
                <Globe className="w-4 h-4" />
            ...
            <Card className="w-full overflow-hidden bg-white/40 backdrop-blur-sm shadow-2xl border-primary/10 rounded-2xl min-h-[60vh] flex flex-col items-center justify-center">
              <CardContent className="p-0 relative w-full flex flex-col items-center justify-center">
                ...
                <div className="w-full flex flex-col items-center justify-center overflow-auto py-8">
                  <Document file={pdfPath} onLoadSuccess={onDocumentLoadSuccess} onLoadError={onDocumentLoadError} loading={null} className="flex flex-col items-center">
                    <Page pageNumber={pageNumber} renderTextLayer={false} renderAnnotationLayer={false} className="shadow-xl" loading={null} width={viewerWidth} />
                  </Document>
                  {!isLoading && !numPages && (
                    <div className="p-8 text-center text-red-600">
                      <p>Sorry, we couldn’t load the brochure.</p>
                      <a href="/brochures/english.pdf" target="_blank" rel="noopener noreferrer" className="underline text-primary">Open the PDF directly</a>
                    </div>
                  )}
                </div>
                {numPages && (
                  <div className="bg-white/90 backdrop-blur-sm border-t border-primary/10 w-full p-4 flex items-center justify-between z-10 sticky bottom-0">
                    <Button variant="outline" size="sm" onClick={() => changePage(-1)} disabled={pageNumber <= 1} className="gap-1 rounded-full">
                      <ChevronLeft className="w-4 h-4" /> Previous
                    </Button>
                    <span className="text-sm font-medium text-foreground">Page <span className="text-primary">{pageNumber}</span> of {numPages}</span>
                    <Button variant="outline" size="sm" onClick={() => changePage(1)} disabled={numPages ? pageNumber >= numPages : true} className="gap-1 rounded-full">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              <Button variant="ghost" asChild className="text-muted-foreground hover:text-primary rounded-full">
                <a href={pdfPath} target="_blank" rel="noopener noreferrer">
                  <Globe className="w-4 h-4 mr-2" /> View Full Screen
                </a>
              </Button>
