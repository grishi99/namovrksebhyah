...
export default function TreeFormPage() {
  ...
  const validateForm = (returnErrors = false) => {
    ...
    // Only require transaction ID in new submission mode, not in edit mode
    if (!isEditMode && !transactionId.trim()) newErrors.transactionId = true;
    // In edit mode, transaction ID should have a value from existing submission
    if (isEditMode && !transactionId.trim()) newErrors.transactionId = true;
      ...
      <main className={`flex flex-col items-center justify-center py-12 px-4 ${(user && !isUserLoggedInAndVerified) ? 'blur-sm' : ''}`}>
        ...
        <Card className="w-full max-w-4xl mb-8 text-center">
          <CardContent className="p-6 space-y-4">
            ...
            <Separator />
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-primary">Vṛkṣāropaṇa Mahotsava 2.0</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="w-full space-y-8">
                  ...
                  <div className="space-y-4 pt-4">
                    <h3 className="text-2xl font-semibold">Adoption Plans</h3>
